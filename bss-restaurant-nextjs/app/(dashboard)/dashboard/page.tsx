...
export default async function Dashboard() {
  ...
  // 3. Mapping data for the Stat Cards
  const stats = [
    {
      label: "Total Revenue",
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-8">
        ...
        <Box className="xl:col-span-2 overflow-hidden shadow-md bg-white/90 rounded-md">
          ...
          <TableContainer>
            <Table sx={{ minWidth: 600 }}>
              <TableHead className="bg-slate-100/80">
                <TableRow>
        ...
        <Box className="bg-white rounded-[5px] shadow-md overflow-hidden">
          ...
          <TableContainer>
            <Table size="small">
              <TableBody>
                {data.topSellingFoods.map((food: any, index: number) => (
                  <TableRow
                    key={food.id}
                    hover
                  ...
                  >
                    ...
                    <TableCell sx={{ width: 60 }}>
                      <Avatar
                        variant="rounded"
                        src={`${process.env.IMG_URL}/images/food/${food.image}`}

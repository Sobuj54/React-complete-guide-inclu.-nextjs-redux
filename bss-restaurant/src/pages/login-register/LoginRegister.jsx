import { ChefHat, User, Lock, Eye, EyeOff, ArrowRight } from "lucide-react"; // All original icons + User
...
export default function AuthPage() {
  ...
  const onSubmit = (data) => {
    console.log("Submitting to BSS API:", data);
    login(data);
  };
    ...
    <div className="flex w-full min-h-screen bg-slate-50">
      ...
      <div className="relative hidden w-1/2 overflow-hidden bg-orange-600 lg:flex">
        ...
        <div className="relative z-20 flex flex-col justify-between w-full p-12 text-white">
          ...
          <p className="text-sm text-slate-300">© 2026 BSS Restaurant Group</p>
        </div>
      </div>
        ...
        <div className="w-full max-w-md space-y-8">
          ...
          <form className="space-y-5" onSubmit={handleSubmit(onSubmit)}>
            ...
            <button
              type="submit"
              disabled={isLoading}
              className="flex items-center justify-center w-full gap-2 py-3 font-bold text-white transition-all bg-orange-600 shadow-lg hover:bg-orange-700 rounded-xl disabled:opacity-70 shadow-orange-600/20 group"
            >
              {isLoading ? (
